...
def read_collabs(feed, author_list, cl):
    collab_list = []
    for entry in feed.entries:
        title = entry.title
...
def send_query(search_query, start, max_results = -1):
...
def query_all(search_query = "all", request = -1, batch = 1000):
    ...
    trial_feed = send_query(search_query, 0, 1)
    ...
    while totalresults < req: #(t == batch and totalresults < req) or (request == -1 and totalresults < feedtotal):
        feed = send_query(search_query, totalresults, batch)
        al = read_authors(feed, al)
        cl = read_collabs(feed, author_list, cl)
        t = len(feed.entries)
    ...
    collab_df.write.mode('overwrite') \
